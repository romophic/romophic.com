...
vi.mock('./content/posts', () => ({
  ...
  getAdjacentPosts: vi.fn().mockResolvedValue({ newer: null, older: null, parent: null }),
...
vi.mock('./content/authors', () => ({
  parseAuthors: vi.fn().mockResolvedValue([{ id: 'author1', name: 'Author One' }]),
}))

vi.mock('./content/toc', () => ({
  getTOCSections: vi.fn().mockResolvedValue([{ id: 'section-1', title: 'Section 1', depth: 2 }]),
}))

vi.mock('./content/links', () => ({
  getBacklinks: vi.fn().mockResolvedValue([{ id: 'referrer', title: 'Referrer Post' }]),
}))

describe('data-utils Specification', () => {
  describe('getPostPageData', () => {
    it('should aggregate data correctly for a standalone post', async () => {
      const mockPost = { id: 'standalone', data: { authors: ['author1'] } } as unknown as CollectionEntry<'blog'>
    ...
    it('should aggregate data correctly for a series parent post', async () => {
      const mockPost = { id: 'parent', data: {} } as unknown as CollectionEntry<'blog'>
    ...
    it('should aggregate data correctly for a subpost', async () => {
      const mockPost = { id: 'parent/child', data: {} } as unknown as CollectionEntry<'blog'>
    ...
    it('should handle missing authors gracefully', async () => {
      const mockPost = { id: 'no-author', data: {} } as unknown as CollectionEntry<'blog'>
      const data = await getPostPageData(mockPost)
