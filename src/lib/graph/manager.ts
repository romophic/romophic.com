...
export class GraphViewManager {
    ...
    private async fetchData() {
        try {
            ...
            nodes.forEach((n) => {
                if (n.group === 'tag') {
                    tagToCategory.set(n.id, categoryIdx % GRAPH_CONFIG.theme.palettes.dark.length)
    ...
    private initVisualization() {
        ...
        const wrapper = this.container.closest('#graph-wrapper') || this.container.parentElement
        if (wrapper) {
            this.isInteractive = (wrapper as HTMLElement).style.pointerEvents === 'auto'
