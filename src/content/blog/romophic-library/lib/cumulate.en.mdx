...
Calculates the prefix sum to find the sum of continuous segments, rectangles, or cuboids in $O(1)$.
...
`vector` is vector, `matrix` is `vector<vector>`, `tensor` is `vector<vector<vector>>`.
...
returns the sum of the interval $[x1,x2)$ for c1.
