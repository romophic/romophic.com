---
title: 'Making STL Data Structures istream/ostream Compatible'
...
If you copy and paste the [Source](#source) below, you can directly `cin` and `cout` `vector`s, `tuple`s, etc. in C++.

---

By overloading the shift operators for:
- `std::vector`
- `std::deque`
- `std::list`
- `std::pair`
...
It recursively expands the structs, allowing input and output processing corresponding to the number of elements, such as in `vector<pair<int,int>>`.
